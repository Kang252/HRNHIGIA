...
namespace NHIGIA.Web.Controllers
{
    ...
    {
        protected HrmUserAccountModel CurrentHrmUser
        {
            get
            {
                var current = HrmSession.Current;
                if (current == null && User != null && User.Identity.IsAuthenticated)
                {
                    current = HrmDataStore.Instance.FindUser(User.Identity.Name);
                    if (current != null)
                    {
                        current.PasswordHash = null;
                        current.PasswordSalt = null;
                        HrmSession.Current = current;
                    }
                }
                return current;
            }
