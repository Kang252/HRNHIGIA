...
public sealed class HanetWebhookController : Controller
{
    ...
    public async Task<IActionResult> Receive()
    {
        try
        {
            ...
            if (!settings.IsEnabled || string.IsNullOrWhiteSpace(settings.WebhookSecret) || !FixedEquals(settings.WebhookSecret, suppliedSecret)) return Unauthorized();
            ...
            var eventKey = Value(payload, "id", "eventId", "eventID") ?? Sha256((body ?? payload.ToJsonString()) + "|" + personId + "|" + aliasId + "|" + checkTime.ToString("O"));
    ...
    private static bool TryParseTime(string value, out DateTime result)
    {
        if (long.TryParse(value, out var epoch))
        {
            ...
            result = DateTimeOffset.FromUnixTimeSeconds(epoch).LocalDateTime;
            return true;
        }
